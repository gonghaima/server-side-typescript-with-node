...
abstract class App {
  ...
  static start_server(): void {
    ...
    server.get("/list-vehicle", App.list_vehicle);
    server.get("/", (req, res) => {
      res.send("call /make-vehicle to create a vehicle");
    });
    server.listen(App.port, () => {
      console.log("Listening on port " + App.port);
    });
  }
  static list_vehicle(request: express.Request, response: express.Response) {
    SimpleAPI.list_vehicles().then(data => response.send(data));
  }
  ...
  static make_vehicle(request: express.Request, response: express.Response) {
    ...
    if (!request.query.class || !types.includes(request.query.class)) {
      return response.send(
        "class parameter must be one of [ " + types.join(" | ") + " ]"
      );
    }

    SimpleAPI.make_vehicle(request.query.class).then(vehicle => {
      response.json(vehicle);
    });
  }

  static display_help() {
    console.log("usage: index.ts [ start-server | list-vehicles ]");
  }
}
